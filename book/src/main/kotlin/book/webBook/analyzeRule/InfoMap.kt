 ...
 */
@Keep
class InfoMap(val sourceUrl: String,val userid: String): MutableMap<String, String> {
    private var actualMap: MutableMap<String, String>
    var needSave = false
    private var saveTime = 0

    init {
        val cache = CacheManager(userid).get("infoMap_${sourceUrl}")
    ...
    fun saveNow() {
        ...
        CacheManager(userid).put("infoMap_${sourceUrl}", json, saveTime)
        needSave = false
    }
